...
{: #tutorial-provision-mongodb-install-cli}
{: step}
...
2. Provision your {{site.data.keyword.databases-for-mongodb}} instance
   ...
   - **Service** - For {{site.data.keyword.databases-for-mongodb}}, the service ID is `databases-for-postgresql`. Choose the correct Service ID for your deployment.
   ...
   - **Admin Password** - The {{site.data.keyword.databases-for-mongodb}} service is provisioned with an admin user, so you can manage PostgreSQL by using its command-line tool, `psql`. For more information, see [Setting the Admin Password](/docs/databases-for-postgresql?topic=databases-for-postgresql-admin-password).
