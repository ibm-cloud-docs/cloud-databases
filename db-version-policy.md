...
| Elasticsearch | Major versions are the first number in a `release.version.maintenance` version number.| TBD | Backup taken and access removed |  
...
| RabbitMQ | Major versions are the first two numbers in a `major.x.patch` version number. | v3.9, 21 September 2023 | Backup taken and access removed |  
